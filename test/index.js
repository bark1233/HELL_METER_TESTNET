...
contract('WTHell', accounts => {
  let token, wtHell, characters, weapons, priceChecker, randoms;
  beforeEach(async () => {
    token = await HellToken.deployed();

    wtHell = await WTHell.new();
    characters = await Characters.new();
    weapons = await Weapons.new();
    priceChecker = await BasicPriceOracle.new();
    randoms = await DummyRandoms.new();

    const GAME_ADMIN = await wtHell.GAME_ADMIN();

    await Promise.all([
      wtHell.initialize(token.address, characters.address, weapons.address, priceChecker.address, randoms.address),
      priceChecker.initialize().then(() => priceChecker.setCurrentPrice(web3.utils.toWei('0.2', 'ether'))),
      characters.initialize().then(() => characters.grantRole(GAME_ADMIN, wtHell.address)),
      weapons.initialize().then(() => weapons.grantRole(GAME_ADMIN, wtHell.address)),
      randoms.initialize().then(() => randoms.setMain(wtHell.address)),
      token.approve(wtHell.address, web3.utils.toWei('100', 'ether')),
    ]);
  });

  describe('mintCharacter', () => {
    it('should work', async () => {
      const { tx } = await wtHell.mintCharacter({ from: accounts[0] });
