...
contract('NFTMarket', accounts => {
  let market, hell, characters;
  beforeEach(async () => {
    hell = await HellToken.deployed();
    characters = await Characters.deployed();

    market = await NFTMarket.new();

    await market.initialize(hell.address, accounts[2]);
    ...
    await hell.transferFrom(hell.address, accounts[1], web3.utils.toWei('10', 'ether'));
  });
  ...
  it('supports buying', async () => {
    ...
    const accountHellBalancesPre = await Promise.all(
      accounts.map(acc => hell.balanceOf(acc).then(toBN))
    );

    await hell.approve(market.address, web3.utils.toWei('6', 'ether'), { from: accounts[1] });
    ...
    const accountHellBalancesPost = await Promise.all(
      accounts.map(acc => hell.balanceOf(acc).then(toBN))
    );

    const accountHellBalanceDiffs = accountHellBalancesPost.map((bn, i) =>
      bn.minus(accountHellBalancesPre[i]).div(web3.utils.unitMap['ether'])
    );

    assert.strictEqual(
      accountHellBalanceDiffs[0].toFixed(1),
      '5.0',
      'Expected seller to recieve original price'
    );
    assert.strictEqual(
      accountHellBalanceDiffs[1].toFixed(1),
      '-5.5',
      'Expected buyer to spend original price plus tax'
    );
    assert.strictEqual(
      accountHellBalanceDiffs[2].toFixed(1),
      '0.5',
