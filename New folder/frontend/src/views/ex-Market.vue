              ...
              <weapon-grid
                ...
                <template #above="{ weapon: { id } }">
                  <span class="d-block text-center" v-if="nftPricesById[id]">
                    <strong>Price</strong>: {{ convertWeiToHell(nftPricesById[id]) | maxDecimals(2) }} HELL
                  </span>
              ...
              <character-list
                ...
                <template #above="{ character: { id } }">
                  <div class="token-price">
                    <span class="d-block text-center" v-if="nftPricesById[id]">
                      {{ convertWeiToHell(nftPricesById[id]) | maxDecimals(2) }} HELL
                    </span>
                    <span class="d-block text-center" v-else>Loading price...</span>
                  </div>
                </template>
...
export default Vue.extend({
  ...
  methods: {
    ...
    async addListingForNft() {
      ...
      const sellFor = await (this as any).$dialog.prompt({ title: `Sell ${this.activeType}`, text: 'Sell Price (HELL)' });
      if(!sellFor) return;
      ...
      const results = await this.addMarketListing({
        ...
        price: this.convertHellToWei(sellFor)
      });

      this.selectedNftId = null;
      this.waitingMarketOutcome = false;
      this.marketOutcome = 'Successfully listed '
        +this.activeType+' '+results.nftID+' for '+this.convertWeiToHell(results.price)+' HELL';
    },
    ...
    async updateNftListingPrice() {
      ...
      const results = await this.changeMarketListingPrice({
        nftContractAddr: this.contractAddress,
        tokenId: this.selectedNftId,
        newPrice: this.convertHellToWei(sellFor)
      });

      this.selectedNftId = null;
      this.waitingMarketOutcome = false;
      this.marketOutcome = 'Successfully changed price for '
        +this.activeType+' '+results.nftID+' to '+this.convertWeiToHell(results.newPrice)+' HELL';
    },
    ...
    async purchaseNft() {
      ...
      const hellChainPrice = this.convertStringToDecimal(this.convertWeiToHell(price), 2);
      const hellUIPrice = this.convertStringToDecimal(this.convertWeiToHell(this.nftPricesById[this.selectedNftId]), 2);

      if(hellChainPrice !== hellUIPrice) {
    ...
    async searchAllCharacterListings(page: number) {
      ...
      const url = new URL('https://api.wthell.io/static/market/character');
      const params = {
    ...
    async searchAllWeaponListings(page: number) {
      ...
      const url = new URL('https://api.wthell.io/static/market/weapon');
      const params = {
    ...
    convertWeiToHell(wei: string) {
      return Web3.utils.fromWei(
        wei,
        'ether'
      );
    },
    convertHellToWei(hell: string) {
