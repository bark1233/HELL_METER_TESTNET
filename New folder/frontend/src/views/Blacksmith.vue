<template>
  <div class="body main-font">
    ...
    <div class="blank-slate" v-if="ownWeapons.length === 0">
      ...
      <big-button
        class="button"
        :mainText="`Forge sword for ${forgeCost} HELL`"
        @click="onForgeWeapon"
      />
    </div>
          ...
          <div class="d-flex justify-content-flex-end ml-auto">
            ...
            <b-button
              ...
              <span v-if="!disableForge" class="gtag-link-others" tagname="forge_weapon">
                Forge ({{ forgeCost }} HELL) <i class="fas fa-plus"></i>
              </span>
            </b-button>
            ...
            <b-modal hide-footer ref="forge-details-modal" title="Forge Percentages">
              <div>
                5+ star @ 1% chance. Estimated cost {{Number.parseFloat(forgeCost * (1/0.01)).toFixed(2)}} HELL.
              </div>
              <div>
                4+ star @ 6% chance. Estimated cost {{Number.parseFloat(forgeCost * (1/0.06)).toFixed(2)}} HELL.
              </div>
              <div>
                3+ star @ 21% chance. Estimated cost {{Number.parseFloat(forgeCost * (1/0.21)).toFixed(2)}} HELL.
              </div>
              <div>
                2+ star @ 56% chance. Estimated cost {{Number.parseFloat(forgeCost * (1/0.56)).toFixed(2)}} HELL.
              </div>
              <div>
                1+ star @ 100% chance.
              </div>
        ...
        <div class="d-flex justify-content-space-between">
          ...
          <div class="d-flex justify-content-flex-end ml-auto">
            <b-button
              ...
              :disabled="canReforge"
              v-tooltip="'Forge new weapon'">
              Confirm Reforge ({{ reforgeCost }} HELL)
            </b-button>
          </div>
        </div>
...
export default {
  ...
  async created() {
    const forgeCost = await this.contracts.WTHell.methods.mintWeaponFee().call({ from: this.defaultAccount });
    const hellForgeCost = await this.contracts.WTHell.methods.usdToHell(forgeCost).call();
    this.forgeCost = BN(hellForgeCost).div(BN(10).pow(18)).toFixed(4);

    const reforgeCost = await this.contracts.WTHell.methods.reforgeWeaponFee().call({ from: this.defaultAccount });
    const hellReforgeCost = await this.contracts.WTHell.methods.usdToHell(reforgeCost).call();
    this.reforgeCost = BN(hellReforgeCost).div(BN(10).pow(18)).toFixed(4);
  },
