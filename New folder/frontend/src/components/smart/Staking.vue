      ...
      >
        <div class="reward-claim-inner-wrapper">
          ...
          <p class="center-text selectable">
            {{ currentRewardEarned.toFixed(18) }} HELL
          </p>
      ...
      <div class="stakePage medium-dark-bg">
        ...
        >
          ...
          <span class="gold-text" v-else-if="canStakeUnclaimedRewards">
            Stake all of unclaimed rewards ({{ formattedHellRewards }} HELL)
          </span>
          <span class="gold-text" v-else>
...
export default {
  ...
  computed: {
    ...mapState(['defaultAccount', 'staking', 'hellRewards']),
    ...
    stakingTokenName() {
      return this.stakeType === 'hell' || this.stakeType === 'hell2' ? 'HELL' : 'HELL-WMTR';
    },
    ...
    canStakeUnclaimedRewards() {
      return !this.loading && new BN(this.hellRewards).gt(0);
      // return true;
    },

    formattedHellRewards() {
      const b = new BN(this.hellRewards);
