<template>
  <div class="body main-font">
    ...
    <div v-if="ownCharacters.length === 0" class="blank-slate">
      ...
      <big-button
        class="button"
        :mainText="`Recruit character for ${recruitCost} HELL`"
        @click="onMintCharacter"
        tagname="recruit_character"
      />
      <div v-if="formatHell() < recruitCost" >
        <br>
        You can buy more HELL from <a v-bind:href="`${getExchangeUrl}`" target="_blank">here</a>.
      </div>
    </div>
        ...
        <div v-if="ownCharacters.length > 0">
          <div class="d-flex justify-content-space-between">
            ...
            <b-button
              ...
              Recruit ({{ recruitCost }} HELL) <i class="fas fa-plus"></i>
            </b-button>
          </div>
...
export default {
  computed: {
    ...mapState(['characters', 'maxStamina', 'currentCharacterId', 'defaultAccount', 'hellBalance']),
    ...mapGetters([
      'contracts',
      'ownCharacters',
      'ownWeapons',
  ...
  async created() {
    const recruitCost = await this.contracts.WTHell.methods.mintCharacterFee().call({ from: this.defaultAccount });
    const hellRecruitCost = await this.contracts.WTHell.methods.usdToHell(recruitCost).call();
    this.recruitCost = BN(hellRecruitCost).div(BN(10).pow(18)).toFixed(4);
  },
  ...
  methods: {
    ...
    formatHell() {
      return Web3.utils.fromWei(this.hellBalance, 'ether');
    },
  },
