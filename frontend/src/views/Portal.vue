<template>
  <div class="body main-font">
    <div class="blank-slate">
      Portal
      <div>
        COMING SOON <a href="#" target="_blank">null</a>.
      </div>
      <br />
      <div class="sub-container" v-if="transactionResults != null && !waitingResults">
        We'll get your MTR to you ASAP!
      </div>
      <div class="loading-container waiting" v-if="waitingResults">
        <i class="fas fa-spinner fa-spin"></i>
        Waiting for transaction results...
      </div>
      <div class="blank-slate" v-if="userAccount === null && !has5HellBalance">
        In order to use the Portal, you will need at least <b>5 HELL!</b> Please add some here:
        <a v-bind:href="`${getExchangeUrl}`" target="_blank">Swap HELL/MTR</a>
      </div>
      <div class="blank-slate" v-if="userAccount === null && has5HellBalance">
        Connect WAX wallet
        <br />
...
export default {
  ...
  computed: {
    ...mapState(['defaultAccount', 'hellBalance', 'inGameOnlyFunds', 'hellRewards']),
    ...mapGetters(['getExchangeUrl', 'hasStakedBalance']),

    has5HellBalance() {
      // Has all 'pools' of HELL to be used later.
      // return Web3.utils.fromWei(this.hellBalance, 'ether') >= 5 || Web3.utils.fromWei(this.inGameOnlyFunds, 'ether') >= 5  || Web3.utils.fromWei(this.hellRewards, 'ether') >= 5 || this.hasStakedBalance;
      return Web3.utils.fromWei(this.hellBalance, 'ether') + Web3.utils.fromWei(this.hellRewards, 'ether') >= 5;
    },
  },
