...
import { networks as hellStakingRewardsNetworks } from '../../build/contracts/HellStakingRewardsUpgradeable.json';
...
import { networks as hellTokenNetworks } from '../../build/contracts/HellToken.json';
...
import { abi as wtHellAbi, networks as wtHellNetworks } from '../../build/contracts/WTHell.json';
...
const stakingContractAddressesFromBuild: Partial<Record<StakeType, Partial<StakingContractEntry>>> = {
  hell: {
    stakingRewardsAddress: (hellStakingRewardsNetworks as Networks)[networkId]?.address,
    stakingTokenAddress: (hellTokenNetworks as Networks)[networkId]?.address
  },
  lp: {
...
async function setUpStakingContracts(web3: Web3) {
  ...
  const hellTokenAddress = process.env.VUE_APP_HELL_TOKEN_CONTRACT_ADDRESS || (hellTokenNetworks as Networks)[networkId]!.address;
  const HellToken = new web3.eth.Contract(erc20Abi as Abi, hellTokenAddress);
...
export async function setUpContracts(web3: Web3): Promise<Contracts> {
  ...
  const wtHellContractAddr = process.env.VUE_APP_WTHELL_CONTRACT_ADDRESS || (wtHellNetworks as Networks)[networkId]!.address;

  const WTHell = new web3.eth.Contract(wtHellAbi as Abi, wtHellContractAddr);
  const [charactersAddr, weaponsAddr, randomsAddr] = await Promise.all([
    WTHell.methods.characters().call(),
    WTHell.methods.weapons().call(),
    WTHell.methods.randoms().call(),
  ]);
  ...
  return {
    ...stakingContracts,
    WTHell, Randoms, Characters, Weapons,
    ...raidContracts,
    ...marketContracts,
    WaxBridge,
  };
