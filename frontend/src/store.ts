...
import { approveFee, getFeeInHellFromUsd } from './contract-call-utils';
...
interface StakingContracts {
  ...
  RewardToken: Contracts['HellToken'],
}
...
export function createStore(web3: Web3) {
  return new Vuex.Store<IState>({
    state: {
      ...
      staking: {
        hell: { ...defaultStakeState },
        hell2: { ...defaultStakeState },
        lp: { ...defaultStakeState },
        lp2: { ...defaultStakeState }
      },
      stakeOverviews: {
        hell: { ...defaultStakeOverviewState },
        hell2: { ...defaultStakeOverviewState },
    ...
    getters: {
      ...
      getExchangeUrl() {
        return 'https://voltswap.finance/#/swap';
      },
    ...
    mutations: {
      ...
      updateHellBalance(state: IState, { hellBalance }) {
        state.hellBalance = hellBalance;
      },

      updateHellRewards(state: IState, { hellRewards }: { hellRewards: string }) {
        state.hellRewards = hellRewards;
      },
    ...
    actions: {
      ...
      setUpContractEvents({ state, dispatch, commit }) {
        ...
        if (!featureFlagStakeOnly) {
            ...
            state.contracts().Characters!.events.NewCharacter(
              ...
              async (err: Error, data: any) => {
                ...
                await Promise.all([
                  ...
                  dispatch('fetchHellBalance'),
                  dispatch('fetchFightRewardHell'),
          ...
          subscriptions.push(
            state.contracts().WTHell!.events.FightOutcome({ filter: { owner: state.defaultAccount } }, async (err: Error, data: any) => {
              if (err) {
                console.error(err, data);
                return;
              }
          ...
          subscriptions.push(
            state.contracts().WTHell!.events.InGameOnlyFundsGiven({ filter: { to: state.defaultAccount } }, async (err: Error, data: any) => {
              if (err) {
                console.error(err, data);
                return;
              }
          ...
          if(NFTMarket) {
            subscriptions.push(
              NFTMarket.events.PurchasedListing({ filter: { seller: state.defaultAccount } }, async (err: Error, data: any) => {
                ...
                await dispatch('fetchHellBalance');
              })
            );
          }
      ...
      async fetchUserDetails({ dispatch }) {
        const promises = [dispatch('fetchHellBalance'), dispatch('fetchWaxBridgeDetails')];
      ...
      async fetchUserGameDetails({ state, dispatch, commit }) {
        ...
        const [
          ...
          state.contracts().WTHell!.methods.getMyCharacters().call(defaultCallOptions(state)),
          state.contracts().WTHell!.methods.getMyWeapons().call(defaultCallOptions(state)),
      ...
      async updateWeaponIds({ state, dispatch, commit }) {
        ...
        const ownedWeaponIds = await state.contracts().WTHell!.methods.getMyWeapons().call(defaultCallOptions(state));
        commit('updateUserDetails', {
          ownedWeaponIds: Array.from(ownedWeaponIds)
        });
      ...
      async updateCharacterIds({ state, dispatch, commit }) {
        ...
        const ownedCharacterIds = await state.contracts().WTHell!.methods.getMyCharacters().call(defaultCallOptions(state));
        commit('updateUserDetails', {
          ownedCharacterIds: Array.from(ownedCharacterIds)
        });
      ...
      async fetchHellBalance({ state, commit, dispatch }) {
        const { defaultAccount } = state;
        if(!defaultAccount) return;

        await Promise.all([
          (async () => {
            const hellBalance = await state.contracts().HellToken.methods
              .balanceOf(defaultAccount)
              .call(defaultCallOptions(state));

            if(state.hellBalance !== hellBalance) {
              commit('updateHellBalance', { hellBalance });
            }
          })(),
          dispatch('fetchInGameOnlyFunds')
        ]);
      ...
      async fetchInGameOnlyFunds({ state, commit }) {
        const { WTHell } = state.contracts();
        if(!WTHell || !state.defaultAccount) return;

        const inGameOnlyFunds = await WTHell.methods
      ...
      async addMoreHell({ state, dispatch }, hellToAdd: string) {
        if(featureFlagStakeOnly) return;

        await state.contracts().WTHell!.methods.recoverHell(hellToAdd).send({
      ...
      async mintCharacter({ state, dispatch }) {
        ...
        await approveFee(
          state.contracts().WTHell!,
          state.contracts().HellToken,
          state.defaultAccount,
          state.hellRewards,
          defaultCallOptions(state),
          defaultCallOptions(state),
          rotInHellMethods => rotInHellMethods.mintCharacterFee()
        );

        await state.contracts().WTHell!.methods.mintCharacter().send(defaultCallOptions(state));
      ...
      async mintWeapon({ state, dispatch }) {
        ...
        await approveFee(
          ...
          defaultCallOptions(state),
          defaultCallOptions(state),
          rotInHellMethods => rotInHellMethods.mintWeaponFee()
        );

        await state.contracts().WTHell!.methods.mintWeapon().send({
          from: state.defaultAccount,
        });
      ...
      async reforgeWeapon({ state, dispatch }, { burnWeaponId, reforgeWeaponId }) {
        ...
        await approveFee(
          ...
          defaultCallOptions(state),
          defaultCallOptions(state),
          rotInHellMethods => rotInHellMethods.reforgeWeaponFee()
        );

        await state.contracts().WTHell!.methods
          .reforgeWeapon(
            reforgeWeaponId,
            burnWeaponId
          )
      ...
      async fetchTargets({ state, commit }, { characterId, weaponId }) {
        ...
        const targets = await state.contracts().WTHell!.methods
      ...
      async doEncounter({ state, dispatch }, { characterId, weaponId, targetString }) {
        ...
        const res = await state.contracts().WTHell!.methods
          .fight(
            characterId,
            weaponId,
            targetString
      ...
      async fetchStakeOverviewDataPartial({ state, commit }, { stakeType }: { stakeType: StakeType }) {
        ...
        const stakeHellOverviewData: IStakeOverviewState = {
          rewardRate,
          rewardsDuration: parseInt(rewardsDuration, 10),
          totalSupply,
          minimumStakeTime: parseInt(minimumStakeTime, 10),
        };
        commit('updateStakeOverviewDataPartial', { stakeType, ...stakeHellOverviewData });
      },
      ...
      async fetchMarketNftPrice({ state }, { nftContractAddr, tokenId }) {
        ...
        // returns the listing's price in hell wei
        return await NFTMarket.methods
          .getFinalPrice(
            nftContractAddr,
            tokenId
      ...
      async purchaseMarketListing({ state, dispatch }, { nftContractAddr, tokenId, maxPrice }: { nftContractAddr: string, tokenId: string, maxPrice: string }) {
        const { HellToken, NFTMarket, Weapons, Characters } = state.contracts();
        if(!NFTMarket || !Weapons || !Characters) return;

        await HellToken.methods
      ...
      async fetchFightGasOffset({ state, commit }) {
        ...
        const fightGasOffset = await getFeeInHellFromUsd(
          WTHell,
          defaultCallOptions(state),
          rotInHellMethods => rotInHellMethods.fightRewardGasOffset()
        );
      ...
      async fetchFightBaseline({ state, commit }) {
        ...
        const fightBaseline = await getFeeInHellFromUsd(
          WTHell,
          defaultCallOptions(state),
          rotInHellMethods => rotInHellMethods.fightRewardBaseline()
        );
      ...
      async fetchFightRewardHell({ state, commit, dispatch }) {
        const { WTHell } = state.contracts();
        if(!WTHell) return;

        const [hellRewards] = await Promise.all([
          (async () => {
            const hellRewards = await WTHell.methods
              .getTokenRewards()
              .call(defaultCallOptions(state));

            commit('updateHellRewards', { hellRewards });
      ...
      async claimTokenRewards({ state, dispatch }) {
        ...
        await WTHell.methods.claimTokenRewards().send({
          from: state.defaultAccount,
        });
      ...
      async claimXpRewards({ state, dispatch }) {
        ...
        await WTHell.methods.claimXpRewards().send({
          from: state.defaultAccount,
        });
