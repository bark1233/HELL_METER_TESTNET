              ...
              <div class="calculator-earnings">
                ...
                <div class="button-div">
                  ...
                  <b-icon-question-circle class="centered-icon" scale="1.5"
                    v-tooltip.bottom="`Earnings on victory: ${this.stringFormattedHell(this.fightGasOffset)} gas offset +
                    ${this.stringFormattedHell(this.fightBaseline)} per square root of power/1000`"/>
                </div>
              </div>
...
export default Vue.extend({
  ...
  methods: {
    ...
    calculateEarnings() {
      ...
      const averageFightProfit = averageReward * this.hellPrice - fightBnbFee;
      for(let i = 1; i < 8; i++) {
    ...
    getAverageRewardForPower(power: number): number {
      return this.formattedHell(this.fightGasOffset) + (this.formattedHell(this.fightBaseline) * Math.sqrt(power / 1000));
    },
    ...
    getAverageRewardAtLevel(level: number): number {
      return this.formattedHell(this.fightGasOffset) + (this.formattedHell(this.fightBaseline) * (Math.sqrt(CharacterPower(level - 1)/1000)));
    },
    ...
    formattedHell(hell: number): number {
      const hellBalance = Web3.utils.fromWei(hell.toString(), 'ether');
      return new BN(hellBalance).toNumber();
    },

    stringFormattedHell(hell: number): string {
      const hellBalance = Web3.utils.fromWei(hell.toString(), 'ether');
      return new BN(hellBalance).toFixed(6);
    },
