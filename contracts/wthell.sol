...
contract WTHell is Initializable, AccessControlUpgradeable {
    ...
    IERC20 public hellToken;//0x154A9F9cbd3449AD22FDaE23044319D6eF2a1Fab;
    IPriceOracle public priceOracleHellPerUsd;
    IRandoms public randoms;

    function initialize(IERC20 _hellToken, Characters _characters, Weapons _weapons, IPriceOracle _priceOracleHellPerUsd, IRandoms _randoms) public initializer {
        __AccessControl_init();

        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(GAME_ADMIN, msg.sender);

        hellToken = _hellToken;
        characters = _characters;
        weapons = _weapons;
        priceOracleHellPerUsd = _priceOracleHellPerUsd;
        randoms = _randoms;
    ...
    mapping(address => uint256) tokenRewards; // user adress : hell wei
    ...
    event FightOutcome(address indexed owner, uint256 indexed character, uint256 weapon, uint32 target, uint24 playerRoll, uint24 enemyRoll, uint16 xpGain, uint256 hellGain);
    event InGameOnlyFundsGiven(address indexed to, uint256 hellAmount);

    function recoverHell(uint256 amount) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not admin");

        hellToken.transfer(msg.sender, amount);
    }

    function getHellToSubtract(uint256 _inGameOnlyFunds, uint256 _tokenRewards, uint256 _hellNeeded)
        public
        pure
        returns (uint256 fromInGameOnlyFunds, uint256 fromTokenRewards, uint256 fromUserWallet) {

        if(_hellNeeded <= _inGameOnlyFunds) {
            return (_hellNeeded, 0, 0);
        }

        _hellNeeded -= _inGameOnlyFunds;

        if(_hellNeeded <= _tokenRewards) {
            return (_inGameOnlyFunds, _hellNeeded, 0);
        }

        _hellNeeded -= _tokenRewards;

        return (_inGameOnlyFunds, _tokenRewards, _hellNeeded);
    }

    function getHellNeededFromUserWallet(address playerAddress, uint256 hellNeeded)
        public
        view
        returns (uint256 hellNeededFromUserWallet) {

        (,, hellNeededFromUserWallet) = getHellToSubtract(
    ...
    function performFight(
        ...
        uint256 tokens = usdToHell(getTokenGainForFight(targetPower));
    ...
    modifier requestPayFromPlayer(int128 usdAmount) {
        uint256 hellAmount = usdToHell(usdAmount);
        ...
        require(hellToken.balanceOf(msg.sender) >= fromUserWallet,
            string(abi.encodePacked("Not enough HELL! Need ",RandomUtil.uint2str(hellAmount))));
        _;
    }
    ...
    function _payContract(address playerAddress, int128 usdAmount) internal {
        _payContractConverted(playerAddress, usdToHell(usdAmount));
    }
    ...
    function _payContractConverted(address playerAddress, uint256 convertedAmount) internal {
        ...
        hellToken.transferFrom(playerAddress, address(this), fromUserWallet);
    }

    function _payPlayer(address playerAddress, int128 baseAmount) internal {
        _payPlayerConverted(playerAddress, usdToHell(baseAmount));
    }

    function _payPlayerConverted(address playerAddress, uint256 convertedAmount) internal {
        hellToken.transfer(playerAddress, convertedAmount);
    }
    ...
    function giveInGameOnlyFunds(address to, uint256 hellAmount) external restricted {
        totalInGameOnlyFunds = totalInGameOnlyFunds.add(hellAmount);
        inGameOnlyFunds[to] = inGameOnlyFunds[to].add(hellAmount);

        hellToken.transferFrom(msg.sender, address(this), hellAmount);

        emit InGameOnlyFundsGiven(to, hellAmount);
    }

    function _giveInGameOnlyFundsFromContractBalance(address to, uint256 hellAmount) internal {
        totalInGameOnlyFunds = totalInGameOnlyFunds.add(hellAmount);
        inGameOnlyFunds[to] = inGameOnlyFunds[to].add(hellAmount);
    ...
    function giveInGameOnlyFundsFromContractBalance(address to, uint256 hellAmount) external restricted {
        _giveInGameOnlyFundsFromContractBalance(to, hellAmount);
    }

    function usdToHell(int128 usdAmount) public view returns (uint256) {
        return usdAmount.mulu(priceOracleHellPerUsd.currentPrice());
    }
    ...
    function stakeUnclaimedRewards() external {
        ...
        hellToken.approve(address(stakeFromGameImpl), _tokenRewards);
    ...
    function getTotalHellOwnedBy(address wallet) public view returns (uint256) {
        return inGameOnlyFunds[wallet] + getTokenRewardsFor(wallet) + hellToken.balanceOf(wallet);
    }
