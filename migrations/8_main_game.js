...
module.exports = async function (deployer, network) {
  ...
  if (network === 'development' || network === 'development-fork') {
    ...
    hellToken = await IERC20.at('0x154a9f9cbd3449ad22fdae23044319d6ef2a1fab');
  }

  assert(hellToken != null, 'Expected hellToken to be set to a contract');
  ...
  const game = await deployProxy(WTHell, [hellToken.address, charas.address, weps.address, priceOracle.address, randoms.address], { deployer });
